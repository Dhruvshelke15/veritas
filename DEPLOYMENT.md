...
**Free-tier RAM warning (confirmed, not hypothetical)**: TensorFlow + PyTorch (via `sentence-transformers`) + ChromaDB resident at once genuinely OOMs Render's free instance type — Render's own monitoring flagged this on the first real deploy ("exceeded its memory limit... automatic restart"). `render.yaml` defaults to `VERITAS_CLASSIFIER_ENABLED=false` to avoid ever importing TensorFlow in production, since routing already fails open without a classifier (`app/rag/routing.py`) — you lose the out-of-scope pre-filter and the advice-seeking disclaimer, not the citation-validation refusal backstop that actually keeps answers grounded. If you'd rather keep the classifier, upgrade to a paid instance type with more RAM instead and set `VERITAS_CLASSIFIER_ENABLED=true`.
